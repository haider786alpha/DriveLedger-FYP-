...
const AddRepair = () => {
  ...
  const fetchData = async () => {
    try {
      const [carsRes, repairsRes] = await Promise.all([
      ...
      console.error("Error fetching repair form data:", error);
      ...
      showToast(
        "error",
        "Data Load Failed",
        "Could not load cars and repairs. Please refresh and try again."
      );
    }
  };
  ...
  const getRepairErrorMessage = (error) => {
    ...
    return "Repair request could not be created. Please check car, issue, date, cost, status, and bill file.";
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(API_URL("/api/repairs/"), cleanPayload(), {
        headers: {
